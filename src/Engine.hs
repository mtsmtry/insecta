...
makeRules (x:xs) = if isStep then (r:ms, mi) else (ms, r:mi) where 
    ...
    makeRule (FuncExpr (PureExprHead _ kind) [a, b]) = case kind of
        ">>=" -> (True, (a, b))
        "->" -> (False, (a, b))
        f -> error f
...
appSimp m (FuncExpr h as) = FuncExpr h' $ map (appSimp m) as where
    h' = case h of PureExprHead p t -> ExprHead p t $ fromMaybe 0 $ elemIndex t m; _-> h
appSimp _ e = e
...
simplify m e = maybe e (simplify m) $ step m e where
    ...
    step m e = applyByHeadList m heads e where
        simpCompare (ExprHead _ _ a) (ExprHead _ _ b) = a `compare` b
